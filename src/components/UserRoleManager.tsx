...
export const UserRoleManager = () => {
  ...
  const updateUserRole = async (userId: string, newRole: 'admin' | 'moderator' | 'user') => {
    try {
      const { data, error } = await (supabase.rpc as any)('update_user_role', {
        _target_user_id: userId, _new_role: newRole
      });
  ...
  const handleAddUser = async () => {
    ...
    try {
      const { data, error } = await supabase.functions.invoke('admin-users', {
        body: { action: 'create', data: { email: newUserEmail, password: newUserPassword, full_name: newUserFullName || null, role: newUserRole } }
      });
      if (error) throw error;
      if (data?.error) throw new Error(data.error);
      toast({ title: "Success", description: "User created successfully" });
      setNewUserEmail(""); setNewUserPassword(""); setNewUserFullName(""); setNewUserRole('user');
  ...
  const handleToggleUserStatus = async (userId: string, currentStatus: boolean) => {
    try {
      const { data, error } = await (supabase.rpc as any)('toggle_user_status', {
        _user_id: userId, _is_active: !currentStatus
      });
      if (error) throw error;
      if (data) {
        toast({ title: "Success", description: `User ${!currentStatus ? 'enabled' : 'disabled'} successfully` });
        fetchUsers();
      } else {
  ...
  const getRoleBadgeVariant = (role: string, isOwner: boolean) => {
    ...
    switch (role) { case 'admin': return 'default'; case 'moderator': return 'secondary'; default: return 'outline'; }
  };

  const getRoleIcon = (role: string, isOwner: boolean) => {
    if (isOwner) return <Crown className="w-4 h-4" />;
    switch (role) { case 'admin': return <Shield className="w-4 h-4" />; case 'moderator': return <Settings className="w-4 h-4" />; default: return <User className="w-4 h-4" />; }
  };
                ...
                <TableRow key={user.id} className={!user.is_active ? 'opacity-50' : ''}>
                  <TableCell>
                    <div className="font-medium">
                      ...
                      {user.is_owner && <Badge variant="outline" className="ml-2 text-xs">Owner</Badge>}
                    </div>
                  </TableCell>
                  ...
                  <TableCell>
                    <div className="flex gap-2">
                      <Select value={user.role} onValueChange={(newRole: 'admin' | 'moderator' | 'user') => updateUserRole(user.id, newRole)} disabled={user.is_owner || !user.is_active}>
                      ...
                      {!user.is_owner && (
                        <>
                          <Button variant={user.is_active ? "secondary" : "default"} size="sm" onClick={() => handleToggleUserStatus(user.id, user.is_active)}>
                            {user.is_active ? (<><Ban className="w-4 h-4 mr-1" />Disable</>) : (<><CheckCircle className="w-4 h-4 mr-1" />Enable</>)}
                          </Button>
        ...
        <DialogContent>
          ...
          <div className="space-y-4">
            ...
            <div className="space-y-2">
              ...
              <Input id="password" type="password" placeholder="Enter password" value={newUserPassword} onChange={(e) => setNewUserPassword(e.target.value)} />
            </div>
            <div className="space-y-2">
